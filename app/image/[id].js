...
const Modal = () => {
  ...
  function fetchData() {
    fetch(`https://api.artic.edu/api/v1/artworks/${id}`)
      ...
      .then((response) => {
        const startDate = response.data.date_start
          ? response.data.date_start
          : "Unknown";
        const endDate = response.data.date_end
          ? response.data.date_end
          : "Unknown";
        setData({
          id: response.data.id,
  ...
  return !!data.image_id ? (
    <ScrollView contentContainerStyle={styles.wrapper} style={{ flex: 1 }}>
      ...
      <View style={styles.propertyWrapper}>
        ...
        <Text style={{ width: width * 0.9 - 44 }}>{data.medium}</Text>
      </View>
      <View style={styles.horizontalSepartor}></View>
      <View style={styles.propertyWrapper}>
